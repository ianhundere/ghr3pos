...
def test_main(capsys):
    assert ghr3pos.main(['--user=britannic', '--repo=blacklist', '--release', '--pullrequest']) == 0
...
def test_main_user_not_found(capsys):
    assert ghr3pos.main(['--user=ianfundere', '--repo=rpi-k3s', '--pullrequest']) == 0
...
def test_main_repo_not_found(capsys):
    assert ghr3pos.main(['--user=ianhundere', '--repo=lauper', '--release']) == 0
...
def test_main_no_rel(capsys):
    assert ghr3pos.main(['--user=ianhundere', '--repo=ran-jam', '--release']) == 0
    assert capsys.readouterr() == ('\nNo releases available\n','')
@pytest.mark.vcr()
def test_main_no_pr(capsys):
    assert ghr3pos.main(['--user=ianhundere', '--repo=scales-o-rama', '--pullrequest']) == 0
